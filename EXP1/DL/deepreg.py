...
for param in ran:
	for step, scale in step_samples:
		for i in param[1]:
			start = time.time()
			################ Samples #####################################
			# X_f1 = list(np.random.uniform(-50,50,40000))
			# Y_f1 = list(np.random.uniform(-50,50,40000))
			# XY_f1 = list(zip(X_f1, Y_f1))

			X_f1 = [x*(1/scale) for x in list(range(lim_inf*scale, lim_sup*scale, step))]
			X_f1 = np.random.permutation(X_f1)
			Y_f1 = [y*(1/scale) for y in list(range(lim_inf*scale, lim_sup*scale, step))]
			Y_f1 = np.random.permutation(Y_f1)
			XY_f1 = list(zip(X_f1, Y_f1))
