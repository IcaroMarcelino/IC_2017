...
py = [y*(1/scale) for y in list(range(lim_inf*scale, lim_sup*scale, step))]
...
outputs = np.array([f(x,y) for x,y in points],dtype = 'float32')
...
with tf.Session() as sess:
    ...
    train_points = points[:int(train_percent*len(points))]
    train_outputs = outputs[:int(train_percent*len(outputs))]
